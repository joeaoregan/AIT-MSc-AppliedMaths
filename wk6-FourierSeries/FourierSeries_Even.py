...
def Even(k, b, L, t):    # t is the time that changes
    result = 0  # set result to 0
    if k<=0:
        return result
    result+= (b/(k*np.pi)) * np.sin((k*np.pi/L)*t) + Even(k - 2, b, L, t)   # 20 / kPi(Sin(kPi/L * t), call function again - only thing that changes is the term number
...
def evenExample(data):
    ...
    text_file = open("Even_Fourier_Data.csv", "w")  # as we calc each point in the series, put in texxt file
    for i in range(size):
        x_data.append(LL)                       # append with position on the limit
        y_data.append(a0 / 2 + Even(k, b, L, LL))       # a0 = dc average, k = num terms, b = 20, limits -- appended to y axis at the same point time is appended
        text_file.write("{0:0.5f} , {1:0.5f} \n".format(LL, Even(k, b, L, LL)))  # write to text file, start at 5 and increase as we go
        LL+=step
    text_file.close()
...
def loadData():     # no need for option here
    x, y = np.genfromtxt('Even_Data.csv', unpack=True, delimiter=',')    # open csv file, comma used as delimiter to separate data
    return (x, y)   # x into x array, y into y array

def main():
    print("Fourier Series Plots - Even")
    evenExample(loadData())      # no need to input 1
